...
const Projections: React.FC<Props> = ({ 
  ...
  useEffect(() => {
    // Locally computed advice — no external calls. Compares the monthly
    // contribution target against income minus expenses.
    const monthlySurplus = monthlyIncome - (monthlyFixedExpenses + monthlyBudgetedExpenses);
    if (monthlySurplus < monthlyContribution) {
      const gap = (monthlyContribution - monthlySurplus).toFixed(0);
      setAiAnalysis(`Your contribution target is $${gap}/mo more than your current surplus — trim expenses or lower the target.`);
    } else {
      setAiAnalysis("Your current surplus comfortably covers your contribution target. Consider increasing it.");
    }
  }, [finalValue, monthlyIncome, monthlyFixedExpenses, monthlyBudgetedExpenses, monthlyContribution]);
